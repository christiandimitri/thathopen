...
export class FragmentHighlighter implements Disposable {
  ...
  constructor(
    ...
    private controls: THREE.PerspectiveCamera
  ) {}
  ...
  async update() {
    ...
    for (const fragmentID in fragments.list) {
      ...
      await this.addHighlightToFragment(fragment);
  ...
  private async addHighlightToFragment(fragment: Fragment) {
    for (const name in this.highlightMats) {
      if (!fragment.fragments[name]) {
        ...
        if (fragment.blocks.count > 1) {
          ...
          await subFragment.blocks.setVisibility(false);
        }
